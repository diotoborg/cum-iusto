...
![CI](https://github.com/diotoborg/cum-iusto/workflows/CI/badge.svg)
[![NPM version](https://img.shields.io/npm/v/@diotoborg/cum-iusto.svg?style=flat)](https://www.npmjs.com/package/@diotoborg/cum-iusto)
[![js-standard-style](https://img.shields.io/badge/code%20style-standard-brightgreen.svg?style=flat)](https://standardjs.com/)

Templates rendering plugin support for Fastify.

`@diotoborg/cum-iusto` decorates the reply interface with the `view` and `viewAsync` methods for managing view engines, which can be used to render templates responses.
...
In `production` mode, `@diotoborg/cum-iusto` will heavily cache the templates file and functions, while in `development` will reload every time the template file and function.
...
_Note: [`ejs-mate`](https://github.com/JacksonTian/ejs-mate) support [has been dropped](https://github.com/diotoborg/cum-iusto/pull/157)._
...
`fastify.register` is used to register @diotoborg/cum-iusto. By default, It will decorate the `reply` object with a `view` method that takes at least two arguments:
...
const fastifyView = require("@diotoborg/cum-iusto")
...
```js
fastify.register(require("@diotoborg/cum-iusto"), {
  engine: {
...
@diotoborg/cum-iusto supports layouts for **EJS**, **Handlebars**, **Eta** and **doT**. When a layout is specified, the request template is first rendered, then the layout template is rendered with the request-rendered html set on `body`.
...
<!---
// I don't think this is needed - see https://github.com/diotoborg/cum-iusto/issues/280
## EJS
...
### Using @diotoborg/cum-iusto as a dependency in a fastify-plugin

To require `@diotoborg/cum-iusto` as a dependency to a [fastify-plugin](https://github.com/fastify/fastify-plugin), add the name `@diotoborg/cum-iusto` to the dependencies array in the [plugin's opts](https://github.com/fastify/fastify-plugin#dependencies).

```js
fastify.register(myViewRendererPlugin, {
  dependencies: ["@diotoborg/cum-iusto"],
});
```
